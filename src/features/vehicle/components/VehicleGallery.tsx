  ...
  return (
    <div className="mb-8">
      <div className="relative rounded-lg overflow-hidden aspect-[16/9] bg-muted">
        {images.length > 0 && <img src={images[currentImageIndex].image_url} alt={`Vehicle image ${currentImageIndex + 1}`} className="w-full h-full object-cover" />}
        {/* Navigation arrows */}
        {images.length > 1 && (
          <>
            <button
        ...
        <div className="flex space-x-2 mt-4 overflow-x-auto p-1">
          {images.map((image, index) => (
            <button
              ...
              className={`flex-shrink-0 w-20 h-16 rounded-md overflow-hidden transition-all ${index === currentImageIndex ? "ring-2 ring-primary" : "opacity-70 hover:opacity-100"}`}
            >
