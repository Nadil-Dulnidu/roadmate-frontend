...
import { CheckCircle2Icon, ColumnsIcon, GripVerticalIcon, LoaderIcon, ChevronDownIcon } from "lucide-react";
...
const columns: ColumnDef<Booking>[] = [
  ...
  {
    accessorKey: "return date",
    header: "Return Date",
    cell: ({ row }) => <span>{format((row.original.end_date), "MMMM dd, yyyy")}</span>,
  },
  {
    accessorKey: "vehicle",
    header: "Vehicle",
...
export function AllActiveBookingDataTable() {
  ...
  const { isSuccess } = useGetAllBookingQuery({ token: authToken, status: ["ACTIVE"] }, { skip: !userId || !authToken });
  const bookings = useAppSelector(selectAllBookings(authToken, ["ACTIVE"]));
