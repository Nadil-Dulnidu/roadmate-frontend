...
const AllBookingsPage = () => {
  ...
  const { isSuccess } = useGetAllBookingQuery({ token: authToken, status: [] }, { skip: !authToken });
  const allBookings = useAppSelector(selectAllBookings(authToken, []));
  

  return (
    <>
      <Helmet>
        <title>Staff Dashboard - Bookings | RoadMate</title>
          ...
          <div className="flex flex-1 flex-col">
            <div className="@container/main flex flex-1 flex-col gap-2">
              <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                  ...
                  {isSuccess && <AllBookingTable data={allBookings} />}
                </div>
              </div>
            </div>
          </div>
